...
until game:IsLoaded()
--▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼ 新增代码开始 ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼--
local savePath = "frz_achievements.dat"  -- 保存文件名
local encryptionKey = "xJ9#kL!p4qR"      -- 加密密钥（不要修改）
...
-- 保存成就数据到本地文件
local function saveAchievements(data)
    local json = game:GetService("HttpService"):JSONEncode(data)
    local encrypted = xorEncrypt(json, encryptionKey)
    pcall(function()
        writefile(savePath, encrypted)
    end)
end
...
local function loadAchievements()
    ...
    if pcall(function() readfile(savePath) end) then
        local encrypted = readfile(savePath)
        local decrypted = xorEncrypt(encrypted, encryptionKey)
...
function library.new(library, name, theme)
    ...
    Main.Size = UDim2.new(0, 572, 0, 396)
    Main.ZIndex = 1
    Main.Active = true
    Main.Draggable = true
...
function section.Credit(section, imageId, topText, descText, unlockCondition)
    ...
    local savedData = loadAchievements()
    local dataKey = "Achievement_" .. imageId
    local unlocked = savedData[dataKey] or false

    -- 如果已解锁，更新UI
    if unlocked then
        LeftImage.ImageTransparency = 0
        DescLabel.Text = descText .. " (已解锁)"
    end
    -- UI元素
    ...
    local function checkUnlock()
        ...
        if success and result then
            ...
            savedData[dataKey] = true
            saveAchievements(savedData)
